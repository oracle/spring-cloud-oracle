...
NOTE: The OCI GenAI `ChatModel` and `EmbeddingModel` APIs are deprecated and will be replaced by Spring AI integration.
...
`ChatModel` requests are stateless. If your application needs multi-turn conversations, store prior turns per user and include the required context in each prompt.
