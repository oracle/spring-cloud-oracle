...
|spring.cloud.oci.config.federation-endpoint | NA | URI of the endpoint used to acquire tokens. This is optional and applies only when `spring.cloud.oci.config.type` is one of: `INSTANCE_PRINCIPAL`, `RESOURCE_PRINCIPAL`, or `WORKLOAD_IDENTITY`.
