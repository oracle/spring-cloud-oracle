...
Set the config.type to `INSTANCE_PRINCIPAL` as shown here. The Federation endpoint is optional.
...
Set the config.type to `RESOURCE_PRINCIPAL` as shown here. The Federation endpoint is optional.
...
Set the config.type to `WORKLOAD_IDENTITY` as shown here. The Federation endpoint is optional.
